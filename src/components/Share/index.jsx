...
const ShareLink = ({ className, label, color, link, onClick }) => (
  <a className="share-link" href={link} target="_blank" rel="noopener noreferrer" onClick={onClick}>
...
const Share = ({ title, authors }) => {
  ...
  return (
    <BrowserOnly>
      {() => {
        const twitter_authors = authors.map((a) =>
          a.twitter_username ? `@${a.twitter_username}` : a.name
        );
        const tweet = encodeURI(
          `https://twitter.com/intent/tweet?text=${title} by ${twitter_authors.join(', ')} ${
